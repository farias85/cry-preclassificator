 ...
 * <p/>
 * The summation on the right hand side of the equation involves a finite number
 * of terms. The number of previous samples used is the order of the linear
 * prediction.
 * <p/>
 * This class also provides a method to compute LPC cepstra, that is, the
 * cepstra computed from LPC coefficients, as well as a method to compute the
 * bilinear transformation of the LPC
 */
public class LinearPredictor {
     ...
     * Assumes the following sign convention:<br> prediction(x[t]) = Sum_i
     * {Ar[i] * x[t-i]}
     *
     * @param autocor
     ...
     *
     * @param RC double array of reflection coefficients. The RC array must
     * begin at 1 (RC[0] is a dummy value)
    ...
    /**
     * Computes LPC Cepstra from the AR predictor parameters and alpha using a
     * recursion invented by Oppenheim et al. The literature shows the optimal
     * value of cepstral order to be:
     * <p/>
     * <
    ...
    /**
     * Computes a bi-linear frequency warped version of the LPC cepstrum from
     * the LPC cepstrum. The recursive algorithm used is defined in Oppenheim's
     * paper in Proceedings of IEEE, June 1972 The program has been written
     * using g[x,y] = g_o[x,-y] where g_o is the array used by Oppenheim. To
     * handle the reversed array index the recursion has been done DOWN the
     * array index.
     *
     * @param warp is the warping coefficient. For 16KHz speech 0.6 is good
     * valued.
     * @param nbilincepstra is the number of bilinear cepstral values to be
     * computed from the linear frequency cepstrum.
